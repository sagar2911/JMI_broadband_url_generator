...
def chat_with_agent(
    ...
    """
    Chat with the broadband agent.
    
    This is a stateless function - message history should be managed by the caller
    (typically the frontend/UI layer).
    
    Args:
        user_message: The user's input message
        deps: Optional dependencies. If None, uses default dependencies.
    
    Returns:
        AgentOutput with message and URLs
    """
    ...
    try:
        ...
        # Run synchronously - agent.run_sync handles the event loop internally
        # Note: Frontend should pass message_history if maintaining context
        ...
        # Log execution
        logger.info(
            f"Agent run time: {execution_time:.3f}s - "
            f"Generated {len(structured_output.urls)} URLs"
        )
        
        # Log the interaction via dependency
        try:
            deps.logger.log_interaction(
                user_message=user_message,
                agent_response=structured_output.message,
                execution_time=execution_time,
            )
        except Exception as log_exc:
            logger.warning(f"Failed to log interaction: {log_exc}")
        ...
        return AgentOutput(message=error_msg, urls=[])
